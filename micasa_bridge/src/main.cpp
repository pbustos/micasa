...
/** \mainpage RoboComp::micasa_bridge
 *
 * \section intro_sec Introduction
 *
 * The micasa_bridge component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/micasa_bridge --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class micasa_bridge : public RoboComp::Application
{
public:
	micasa_bridge (QString prfx, bool startup_check) { prefix = prfx.toStdString(); this->startup_check_flag=startup_check; }
private:
	void initialize();
	std::string prefix;
	TuplePrx tprx;
...
int ::micasa_bridge::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	try
	{
		...
		{
			// Server adapter creation and publication
			if (not GenericMonitor::configGetString(communicator(), prefix, "VisualElements.Endpoints", tmp, ""))
			{
				cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy VisualElements";
			}
			...
			cout << "[" << PROGRAM_NAME << "]: VisualElements adapter created in port " << tmp << endl;
		}
...
int main(int argc, char* argv[])
{
	...
	::micasa_bridge app(prefix, startup_check_flag);
